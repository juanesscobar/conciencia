...
export default function Tasks() {
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-1">
            {filtered.length} tareas · <span className="text-primary-500">{formatHours(totalHours)}</span> estimadas
          </p>
        </div>
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          <select
            ...
            className="hack-select text-sm flex-1 md:flex-none"
          >
          ...
          <button
            ...
            className="px-4 py-2 bg-primary-600/90 text-bg-950 font-bold rounded-lg hover:bg-primary-500 hover:shadow-neon text-sm transition-all flex-1 md:flex-none"
          >
            + NEW_TASK
          </button>
        </div>
      ...
      {/* Desktop: tabla */}
      <div className="hack-card overflow-hidden hidden md:block">
          ...
          <table className="min-w-full divide-y divide-bg-800">
            <thead className="bg-bg-800">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-primary-400 uppercase tracking-wider">Est.</th>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((task) => (
                  <tr key={task.id} className="hover:bg-bg-800 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-primary-400 font-mono">{formatHours(task.estimated_hours)}</td>
      ...
      <div className="md:hidden space-y-3">
        {filtered.length === 0 ? (
          <div className="hack-card p-6 text-center text-gray-600">No tasks found</div>
        ) : (
          filtered.map((task) => (
            <div key={task.id} className="hack-card p-4 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <p className="font-medium text-gray-200 text-sm leading-snug">{task.title}</p>
                <span className="text-xs text-primary-400 font-mono whitespace-nowrap">{formatHours(task.estimated_hours)}</span>
              </div>
              {task.description && (
                <p className="text-xs text-gray-600 line-clamp-2">{task.description}</p>
              )}
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <span className={`px-2 py-0.5 text-[10px] rounded-full font-medium ${statusColors[task.status]}`}>
                  {task.status.replace(/_/g, ' ')}
                </span>
                <span className={`px-2 py-0.5 text-[10px] rounded-full font-medium ${priorityColors[task.priority]}`}>
                  {task.priority}
                </span>
                <span className="text-[10px] text-gray-600 capitalize">{task.type}</span>
              </div>
      ...
      <div className="hack-card w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto border-primary-500/30 shadow-neon">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-primary-400 mb-1">$ est. horas</label>
              <input
                type="number"
                step="0.5"
                min="0"
